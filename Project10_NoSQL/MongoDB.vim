...
mongoimport -u root -p pw --authenticationDatabase admin --db entertainment --collection movies --file movies.json
...
db.movies.aggregate([{“$group”:{“_id”:"$year"}}, {“moviecount”:{"$sum":1}}]) , {"$limit":1}
db.movies.aggregate([{“$group”:{“_id”:"$year"}}, {“moviecount”:{"$sum":1}}])
db.movies.aggregate([{“$group”:{“_id”:"$year"}}, {“moviecount”:{"$avg":"$Votes"}}]) {"$match": {"$year":"2007"}}
...
mongoexport -u root -p pw --authenticationDatabase admin --db entertainment --collection movies --out partial_data.csv --type=csv --fields _id,title,year,rating,director
