...
map (func)             : returns a new distributed dataset formed by passing each element of the source through a function
...
flatmpa (func)         : similar to map. It can map each input item to zero or more output items. Func should return a Seq rather than a single item


# Action

redunce (func)         : Func takes two arguemtns adn returns one is commutative/ associative/ can be computed correctly in parallel
